...
func (server *Server) DeactivateSiteLater(c *gin.Context) {
	...
	err = server.DB.Debug().Raw("EXEC spAPI_RetailerSiteRelation_CountActiveSite ?", originalSite.OriginalID).Scan(&activeRelation).Error
	if err != nil {
		errList["unmarshal_error"] = "Cannot unmarshal body"
		c.JSON(http.StatusUnprocessableEntity, gin.H{
			"error": errList,
	...
	err = server.DB.Debug().Raw("EXEC spAPI_Terminal_CountActiveSite ?", originalSite.OriginalID).Scan(&activeTerminal).Error
	if err != nil {
		errList["unmarshal_error"] = "Cannot unmarshal body"
		c.JSON(http.StatusUnprocessableEntity, gin.H{
			"error": errList,
	...
	if activeTerminal.SiteCount > 0 {
		errList["linked_retailer"] = "Selected site is still linked to a terminal"
		c.JSON(http.StatusUnprocessableEntity, gin.H{
			"error": errList,
		})
		return
